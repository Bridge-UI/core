...
`useResolveMessage` reads the active adapter from context. **Interpolation and pluralization are the adapter’s job** (vue-i18n, i18next, or a custom `t`). Bridge `setLocale` updates `global.locale` and calls optional `adapter.setLocale` — persistence (localStorage, backend) stays in the app.
