...
and picks `|` forms when `count` is set. Bridge `setLocale` updates
`global.locale` and calls optional `adapter.setLocale` — persistence
...
BridgeUIProvider, DataTable, PasswordField, NumberField, Select, Listbox, Snackbar
