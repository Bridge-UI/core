...
Set `href` to render the row as an anchor. The browser shows the URL on hover, and middle-click opens a new tab. `target` and `rel` are forwarded to that anchor. `linkAs` replaces that anchor. `linkProps` is checked against `linkAs` and forwarded to it. A disabled item keeps the styles but drops the URL.
...
| `linkAs`       | `ElementType`                        | —        | Component rendered in place of the interactive `<a>`. The root stays `as` (`li` or `div`). Ignored while disabled.                                                               |
