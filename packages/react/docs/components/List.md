...
Set `href` to render the row as an anchor. The browser shows the URL on hover, and middle-click opens a new tab. `target` and `rel` are forwarded to that anchor. `linkAs` replaces that anchor. A disabled item keeps the styles but drops the URL.
