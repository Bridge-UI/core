...
Same as `List`, plus `iconOnly`. Sets `iconOnly` when the icon rail is collapsed on desktop. Applies stacked nav chrome (gap, compact rows, nested guide). Override with `iconOnly`. Nested `SidebarList` is hidden while collapsed. `ListSection` labels are hidden while collapsed.

## Props (`SidebarListItem`)

Same as `ListItem` (including `href`, `linkAs`, and `linkProps`), plus `tooltip` / `tooltipPlacement`. Applies compact nav chrome. Tooltips only show while the icon rail is collapsed (`primary`, or `tooltip` to override). A collapsed link keeps `href` on the anchor. `linkAs` replaces that anchor and leaves the row root as `li` or `div`.
