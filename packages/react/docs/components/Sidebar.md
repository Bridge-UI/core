...
Same as `ListItem`, plus `tooltip` / `tooltipPlacement`. Applies compact nav chrome. When the icon rail is collapsed, string `primary` is shown in a tooltip on the whole row.
