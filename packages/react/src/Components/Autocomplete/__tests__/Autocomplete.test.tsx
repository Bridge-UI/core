...
function ControlledAutocomplete(
  ...
  return <Autocomplete {...rest} value={value} onChange={setValue} />;
}
...
test("it should filter options when searchable", async () => {
  render(<Autocomplete searchable options={options} aria-label="Fruit" />);
...
test("it should select from composed ListSection and ListItem children", async () => {
  ...
  render(
    <Autocomplete value="" aria-label="Fruit" onChange={onChange}>
...
test("it should commit free-solo text on Enter by default", async () => {
  ...
  fireEvent.input(combobox, { target: { value: "mango" } });
...
test("it should not commit free-solo text when freeSolo is disabled", async () => {
  ...
  expect(onChange).not.toHaveBeenCalledWith("mango");
});
