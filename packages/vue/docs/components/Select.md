...
<Select overlay="modal" label="Country" :options="countries" />
...
Set `components.Listbox.defaultProps.matchWidth` on `createBridgeUI` to apply this to every Select / Autocomplete menu.
