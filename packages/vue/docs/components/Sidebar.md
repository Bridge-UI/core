...
Same as `ListItem` (including `href`, `link-as`, and `link-props`), plus `tooltip` / `tooltip-placement`. Applies compact nav chrome. Tooltips only show while the icon rail is collapsed (`primary`, or `tooltip` to override). A collapsed link keeps `href` on the anchor. `link-as` replaces that anchor and leaves the row root as `li` or `div`.
